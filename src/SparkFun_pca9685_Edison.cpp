...
// Sets all four motors to the same thing
void pca9685::setFourMotors(float throttle){
	setChlDuty(0, invertAndScale(throttle));
	setChlDuty(1, invertAndScale(throttle));
	setChlDuty(2, invertAndScale(throttle));
	setChlDuty(3, invertAndScale(throttle));
}

// Takes in a percentage, maps it from 0-100, inverts
float pca9685::invertAndScale(float throttle){
	if (throttle > 100.0)
		throttle = 0.0 ;
